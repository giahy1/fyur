...
def search_venues():
  ...
  print('====================search_term '+str(venues))
  now = datetime.now()
  len_venues = len(venues)
  response = VenuesSearch(count=len_venues,data=[])
  for venue in venues:
    num_upcoming_shows = sum(1 for show in venue.shows if show.start_time >= now)
    response.data.append({"id":venue.id,"name": venue.name,"num_upcoming_shows":num_upcoming_shows})
...
def insert_show(data):
  for dt in data:
    venue = Venue.query.filter_by(name =dt['venue_name'] ).first()
    artist = Artist.query.filter_by(name =dt['artist_name'] ).first()
    new_show = Show(venue_id = venue.id, artist_id = artist.id, start_time = dt['start_time'])
